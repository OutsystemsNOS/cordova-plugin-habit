...
public class HabitPlugin extends CordovaPlugin {
  ...
  public boolean execute(String action, JSONArray args, CallbackContext callbackContext) throws JSONException {
    if (action.equals("getDeviceInfo")) {
            this.getDeviceInfo(callbackContext, args.getInt(0), args.getInt(1));
            return true;
        } 
    return false;
    }
   
    private void getDeviceInfo(final CallbackContext callbackContext, final int number, final int imei) {      
      Log.d(TAG, "getDeviceInfo  called. number: " + Integer.toString(number));
      cordova.getThreadPool().execute(new Runnable() {
        public void run() {
          try {
                DeviceHealth.getDeviceInfo(this.cordova.getActivity().getApplicationContext(), this.cordova.getActivity(), number, imei, new DeviceInfoCallback() {
                @Override
